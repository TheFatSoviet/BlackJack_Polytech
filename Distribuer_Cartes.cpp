...
#include <string>
#include <vector>
#include "Joueur.h" // Assurez-vous d'inclure l'en-tête correct pour la classe Joueur.
...
void Distribuer_Cartes(std::vector<Joueur>& joueurs) {
    ...
    for (auto& joueur : joueurs) {
        if (cartes.size() >= 2) { // Assurez-vous qu'il reste assez de cartes pour distribuer.
    ...
    if (fileOut.is_open()) {
        for (size_t i = 0; i < cartes.size(); ++i) {
            fileOut << cartes[i];
            if (i != cartes.size() - 1) { // Ajoute une virgule entre les cartes, sauf après la dernière.
                fileOut << ",";
            }
        }
